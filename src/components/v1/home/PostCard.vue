        ...
        <div class="author-unit" :class="post.author.role">
          ...
          <template v-if="post.author.role && post.author.role !== 'user'">
            ...
            <span class="role-label">
              <font-awesome-icon
                v-if="post.author.role === 'super_admin'"
                icon="gem"
              />
              <font-awesome-icon
              ...
              />
              <font-awesome-icon
                v-if="post.author.role === 'system'"
                icon="circle-check"
              />
              <font-awesome-icon
                v-if="post.author.role === 'vip'"
                icon="crown"
              />
              {{
...
const catIndex = computed(
  () => Math.max(0, CATEGORIES.indexOf(props.post.category)) % 5
);
...
const readingMinutes = computed(() =>
  Math.max(3, Math.round(props.post.description.length / 42))
);
const commentsEstimate = computed(() =>
  Math.max(8, Math.round(props.post.views / 95))
);
...
.post-card {
  ...
  &:hover {
    ...
    box-shadow: 0 20px 25px -5px rgba(37, 99, 235, 0.08),
      0 10px 10px -12px rgba(37, 99, 235, 0.08);
