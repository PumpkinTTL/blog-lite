<template>
  <header class="fixed top-0 left-0 right-0 z-50 transition-colors duration-300"
    :class="isDark ? 'bg-[#0F172A] border-gray-800' : 'bg-white border-gray-200'" style="border-bottom-width: 1px;">
    <nav class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
      <div class="flex items-center gap-2 sm:gap-4 h-14 sm:h-16">
        ...
        <!-- Desktop Nav Panel -->
        <div class="hidden md:flex items-center gap-1 px-2 py-1.5 rounded-lg transition-all"
          :class="isDark ? 'bg-gray-800/40 border border-gray-700/50' : 'bg-gray-50 border border-gray-200/50'">
          <a v-for="item in navItems" :key="item.name" :href="item.href"
            ...
            :class="isDark ? 'text-gray-300 hover:text-white hover:bg-gray-700/60' : 'text-gray-700 hover:text-gray-900 hover:bg-white'"
            @click.prevent="handleNavClick(item)">
            <font-awesome-icon :icon="item.icon" class="text-sm"
              :class="isDark ? 'group-hover:text-blue-400' : 'group-hover:text-blue-600'" />
            <span>{{ item.name }}</span>
          </a>
        </div>
        ...
        <div class="flex items-center gap-1 sm:gap-2">
          <!-- Search Button -->
          <button class="p-1.5 sm:p-2 rounded-lg transition-all cursor-pointer"
            :class="isDark ? 'text-gray-400 hover:text-white hover:bg-gray-800/60' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'"
            @click="openSearch">
            <font-awesome-icon icon="search" class="text-sm sm:text-base" />
          </button>

          <!-- Theme Toggle (Desktop only) -->
          <button class="hidden sm:block p-2 rounded-lg transition-all cursor-pointer"
            :class="isDark ? 'text-gray-400 hover:text-white hover:bg-gray-800/60' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'"
            @click="toggleTheme">
          ...
          <button class="hidden sm:block relative p-2 rounded-lg transition-all cursor-pointer"
            :class="isDark ? 'text-gray-400 hover:text-white hover:bg-gray-800/60' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'"
          ...
          <div class="relative" ref="userMenuRef">
            <button
              ...
              :class="isDark ? 'bg-gray-800/60 hover:bg-gray-800' : 'bg-gray-100 hover:bg-gray-200'"
              ...
              <div class="relative">
                <div
                  ...
                  :class="isDark ? 'ring-gray-700 ring-offset-[#0F172A]' : 'ring-gray-200 ring-offset-white'">
              ...
              <span class="text-xs sm:text-sm font-medium username-text max-w-[60px] sm:max-w-none truncate"
                :class="[isDark ? 'text-gray-200' : 'text-gray-700', user?.isVip ? 'vip-shimmer' : '']">{{
                  user?.username || '用户' }}</span>
              <font-awesome-icon icon="chevron-down" class="text-xs transition-transform duration-200"
                :class="[isDark ? 'text-gray-400' : 'text-gray-500', isUserMenuOpen ? 'rotate-180' : '']" />
            </button>

            <!-- User Menu Dropdown -->
            <transition>
              <div v-if="isUserMenuOpen" class="absolute right-0 mt-4 w-60 sm:w-64 rounded-xl overflow-hidden z-50"
                :class="isDark ? 'bg-gray-800/95 backdrop-blur-xl border border-gray-700/50' : 'bg-white/95 backdrop-blur-xl border border-gray-200/50'"
                ...
                <div class="animate__animated animate__fadeInUp px-4 py-4 relative overflow-hidden"
                  ...
                  :class="isDark ? 'bg-gradient-to-br from-gray-900/50 to-gray-800/50' : 'bg-gradient-to-br from-gray-50/50 to-white/50'">
                  <div class="absolute inset-0 bg-gradient-to-br from-blue-500/5 to-transparent"></div>
                  <div class="flex items-start gap-3 relative z-10">
                    <div class="relative flex-shrink-0">
                      <div class="w-12 h-12 rounded-lg overflow-hidden ring-2 ring-offset-2"
                        :class="isDark ? 'ring-gray-700 ring-offset-gray-800' : 'ring-gray-200 ring-offset-white'">
                    ...
                    <div class="flex-1 min-w-0 pt-0.5">
                      <div class="flex items-center gap-1.5 mb-1.5">
                        <p class="text-sm font-bold username-text truncate"
                          :class="[isDark ? 'text-white' : 'text-gray-900', user?.isVip ? 'vip-shimmer' : '']">{{
                            user?.username || '用户' }}</p>
                        <span v-if="user?.isVip"
                          ...
                          :class="isDark ? 'bg-yellow-500/10 text-yellow-400 border border-yellow-500/20' : 'bg-yellow-50 text-yellow-600 border border-yellow-200'">
                          <font-awesome-icon icon="crown" class="text-[8px]" />
                          VIP
                        </span>
                      </div>
                      <p class="text-xs truncate" :class="isDark ? 'text-gray-400' : 'text-gray-500'">{{ user?.email ||
                        'user@example.com' }}</p>
                    </div>
                  </div>
                </div>
                ...
                <div class="py-2 px-2">
                  <a href="#"
                    ...
                    :class="isDark ? 'text-gray-300 hover:text-white hover:bg-gray-700/50' : 'text-gray-700 hover:text-gray-900 hover:bg-gray-50'"
                    @click.prevent="openProfileCenter">
                    <font-awesome-icon icon="user" class="text-sm w-4"
                      :class="isDark ? 'text-gray-400 group-hover:text-blue-400' : 'text-gray-500 group-hover:text-blue-600'" />
                    <span>个人资料</span>
                  </a>
                  <a href="#"
                    class="animate__animated animate__fadeInUp flex items-center gap-2.5 px-3 py-2 mb-0.5 text-sm font-medium rounded-lg transition-all duration-200 cursor-pointer group"
                    ...
                    :class="isDark ? 'text-gray-300 hover:text-white hover:bg-gray-700/50' : 'text-gray-700 hover:text-gray-900 hover:bg-gray-50'">
                    <font-awesome-icon icon="cog" class="text-sm w-4"
                      :class="isDark ? 'text-gray-400 group-hover:text-blue-400' : 'text-gray-500 group-hover:text-blue-600'" />
                    <span>设置</span>
                  </a>
                  ...
                  <a href="#"
                    ...
                    <font-awesome-icon :icon="isDark ? 'sun' : 'moon'" class="text-sm w-4"
                      :class="isDark ? 'text-gray-400 group-hover:text-blue-400' : 'text-gray-500 group-hover:text-blue-600'" />
                  ...
                  <a href="#"
                    ...
                    <font-awesome-icon icon="bell" class="text-sm w-4"
                      :class="isDark ? 'text-gray-400 group-hover:text-blue-400' : 'text-gray-500 group-hover:text-blue-600'" />
                    <span>通知中心</span>
                  </a>
                </div>
                ...
                <div class="animate__animated animate__fadeInUp px-2 pb-2 pt-1 border-t"
                  ...
                  <a href="#"
                    ...
                    :class="isDark ? 'text-red-400 hover:text-red-300 hover:bg-red-500/10' : 'text-red-600 hover:text-red-700 hover:bg-red-50'">
          ...
          <button class="md:hidden p-1.5 rounded-lg transition-all cursor-pointer"
            :class="isDark ? 'text-gray-400 hover:text-white hover:bg-gray-800/60' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'"
    ...
    <transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 -translate-y-2"
      ...
      <div v-if="isMobileMenuOpen" class="md:hidden"
        :class="isDark ? 'bg-gray-900 border-gray-800' : 'bg-gray-50 border-gray-200'" style="border-top-width: 1px;">
        <div class="px-4 py-3 space-y-1">
          <a v-for="item in navItems" :key="item.name" :href="item.href"
            class="flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-lg transition-all cursor-pointer"
            :class="isDark ? 'text-gray-300 hover:text-white hover:bg-gray-800/60' : 'text-gray-700 hover:text-gray-900 hover:bg-white'"
          ...
          <div class="pt-3" :class="isDark ? 'border-t border-gray-800' : 'border-t border-gray-200'">
            <div class="flex items-center gap-3 px-4 py-3 rounded-lg" :class="isDark ? 'bg-gray-800/60' : 'bg-white'">
              ...
              <div>
                <p class="text-sm font-semibold username-text"
                  :class="[isDark ? 'text-white' : 'text-gray-900', user?.isVip ? 'vip-shimmer' : '']">{{ user?.username
                    || '用户' }}</p>
                <p class="text-xs" :class="isDark ? 'text-gray-400' : 'text-gray-500'">{{ user?.email ||
                  'user@example.com' }}</p>
              </div>
            </div>
          </div>
