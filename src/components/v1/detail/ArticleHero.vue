<template>
  <article class="article-hero" :class="{ 'dark-mode': isDark }">
    ...
    <div class="cover-header">
      <img :src="article.cover" :alt="article.title" class="cover-img" loading="lazy" />
    ...
    <div class="hero-body">
      ...
      <div class="tags-row">
        <span v-for="(tag, i) in article.tags.slice(0, 4)" :key="tag" class="tag" :class="`t${i % 4}`">
          #{{ tag }}
        </span>
      </div>
        ...
        <div class="author-block">
          ...
          <div class="author-details">
            <div class="author-name-row">
              ...
              <span v-if="roleLabel" class="role-badge" :class="article.author.role">
                <font-awesome-icon v-if="article.author.role === 'super_admin'" icon="gem" />
                <font-awesome-icon v-if="article.author.role === 'admin'" icon="shield-halved" />
                <font-awesome-icon v-if="article.author.role === 'system'" icon="circle-check" />
                <font-awesome-icon v-if="article.author.role === 'vip'" icon="crown" />
                {{ roleLabel }}
              </span>
            </div>
...
const readTime = computed(() => Math.max(3, Math.ceil(props.article.description.length / 50)))
...
.ai-summary-premium {
  ...
  padding: 24px;
  border-radius: 16px;
  background: linear-gradient(135deg, rgba(244, 244, 255, 0.9) 0%, rgba(243, 232, 255, 0.7) 100%);
...
.sparkle {
  ...
  &.sparkle-1 { animation-delay: 0s; margin-right: 6px; }
  &.sparkle-2 { animation-delay: 1.5s; font-size: 14px; color: #ec4899; }
}
...
.dark-mode {
  ...
  .meta-section { border-color: rgba(255, 255, 255, 0.1); }
  .name { color: #f1f5f9; }
  .publish-date { color: #94a3b8; }
  ...
  /* 黑暗模式 AI摘要依然高级通透 */
  .ai-summary-premium {
    background: linear-gradient(135deg, rgba(88, 28, 135, 0.2) 0%, rgba(126, 34, 206, 0.05) 100%);
    border-color: rgba(192, 132, 252, 0.25);
    box-shadow: 0 8px 32px rgba(0, 0, 0, 0.2);
  }

  .ai-text { color: #e9d5ff; }
}
...
@media (max-width: 768px) {
  ...
  .stats-block { width: 100%; justify-content: flex-start; }
}
</style>
