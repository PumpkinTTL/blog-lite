...
<template>
  <header v-animate class="group relative overflow-hidden rounded-2xl bg-muted animate__animated animate__zoomIn">
    <!-- 封面图:全宽背景,hover时微放大 -->
    ...
    <div class="relative flex min-h-[280px] flex-col justify-end p-5 sm:min-h-[320px] sm:p-7 lg:min-h-[380px] lg:p-9">
      <!-- 分类 + 难度 -->
      <div class="mb-2 flex items-center gap-2 animate__animated animate__fadeInLeft" style="animation-delay: 0.08s">
      ...
      <h1 class="mb-2 max-w-3xl text-2xl font-bold tracking-tight text-white sm:text-3xl lg:text-5xl animate__animated animate__fadeInUp" style="animation-delay: 0.14s">
        {{ article.title }}
      </h1>

      <!-- 描述:适中字号,白色/80 -->
      <p class="mb-3 max-w-2xl text-sm leading-relaxed text-white/80 sm:text-base animate__animated animate__fadeInUp" style="animation-delay: 0.20s">
        {{ article.description }}
      </p>

      <!-- 元信息:单行排列,backdrop-blur -->
      <div class="flex flex-wrap items-center gap-3 rounded-lg bg-black/20 px-3 py-2 text-xs text-white/70 backdrop-blur-sm animate__animated animate__fadeInUp" style="animation-delay: 0.26s">
      ...
      <!-- 标签:底部 -->
      <div v-if="article.tags.length" class="mt-3 flex flex-wrap gap-1.5 animate__animated animate__fadeInUp" style="animation-delay: 0.32s">
        <Badge
